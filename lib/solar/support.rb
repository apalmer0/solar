module Support
  ...
  def to_julian_centuries(t)
    julian_day = julian_day_rational(to_dynamical_time(t))
  ...
  def to_dynamical_time(datetime)
    datetime = datetime.utc

    datetime + to_rational(delta_t(datetime)) / SECONDS_PER_DAY
  end
